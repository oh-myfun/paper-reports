...
- **[HTML] 数学公式处理**：使用 LaTeX 语法书写公式，行内用 `$...$`，独立公式用 `$$...$$`。模板已内置 MathJax 3，无需额外引入。生成 HTML 时注意：在 Python 中构建含公式的字符串时，使用**原始字符串（raw string）或单反斜杠**确保输出文件中 LaTeX 命令前是单个 `\`（如 `\mathcal`），而非双反斜杠 `\\mathcal`。**注意**：MathJax 默认不支持 `\bm` 命令，请使用 `\boldsymbol` 替代。
- **[MD] 数学公式处理**：使用标准 LaTeX 语法 `$...$` 和 `$$...$$`，与主流 Markdown 渲染器兼容。

### 索引页卡片格式（manifest.json）

报告生成后，必须更新 `manifest.json` 中对应条目。卡片格式须遵循以下规则，**所有会话修改时必须遵守**：

- **title**：简洁的英文缩写名，不含中文名或冗长描述。如 `"MiniMind-O"`、`"Seed-TTS"`、`"SupertonicTTS"`。附录页可加 `" 附录"` 后缀。
- **description**：简短一句话介绍论文核心内容，**不得重复 title**。格式为纯描述性文字，不含模型名前缀。例如：
  - ✅ `"大规模语音生成模型族，零样本ICL达到人类水平，含自蒸馏解耦与全扩散变体"`
  - ❌ `"Seed-TTS：高质量多功能语音生成模型族"`（重复了 title）
  - ✅ `"基于扩散语言模型的全语种零样本文本转语音系统"`
  - ❌ `"OmniVoice：基于扩散语言模型的全语种零样本文本转语音"`（重复了 title）
  - ❌ `"FunAudioLLM"`（仅重复了 title，毫无描述信息）
- **tags**：有意义的信息标签数组，用于索引页卡片展示。**不要使用无意义标签如"单页"、"1页"**，应使用模型类型、参数规模等有价值的信息。标签保持缩写形式，悬停提示由索引页 `TAG_TIPS` 映射提供全称+中文。常见标签：
  - 模型类型：`"TTS"`、`"ASR"`、`"VC"`、`"LM"`、`"Omni"`、`"DiT"`、`"SER"`
  - 参数规模：`"18M"`、`"44M"`、`"220M"`、`"0.1B"`、`"2B"` 等（如果论文未公开参数量则不加此标签）
  - 附录页标签：`"Appendix"`
- **year**：论文发表年份（整数），用于索引页年份过滤按钮和卡片右上角年份显示。

**🔴 红线规则：修改 manifest.json 时，只允许添加新条目或修改当前报告对应的条目，绝对不得修改其他已有报告的任何字段（title、description、tags 等）。每次只做增量操作，不要重写整个文件。**
...
```json
{
  "name": "seed-tts.html",
  "title": "Seed-TTS",
  "description": "大规模语音生成模型族，零样本ICL达到人类水平，含自蒸馏解耦与全扩散变体",
  "tags": ["TTS", "VC", "LM", "DiT"]
}
```
...
报告 HTML 页面仅包含内容（头部信息、正文、图片、页脚），不包含任何框架级 UI（返回按钮、适配按钮等）。框架层 UI 统一由 `view/index.html` 提供：

- **左侧**：返回按钮（链接到首页 `../`）
- **右侧**：适配按钮（切换报告内容区宽屏/标准宽度）
- **目录型报告**额外有：侧边栏页面列表 + 上一页/下一页按钮

报告 HTML 页面中**不得**添加 `.top-bar`、`.back-link` 等导航元素，这些由 `view/index.html` 的 toolbar 统一处理。附录页也不需要单独的返回链接。
